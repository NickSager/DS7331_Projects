...
class_report_dict = classification_report(y, y_pred_rf, output_dict=True)
class_report_df = pd.DataFrame(class_report_dict).transpose()
class_report_df.loc["accuracy"] = class_report_df.loc["accuracy"].drop(
    ["precision", "recall"]
)
class_report_df = class_report_df.rename(
    index={"0": "Not Bankruptcy", "1": "Bankruptcy"}
)
class_report_df = class_report_df.round(3)
class_report_df = class_report_df.fillna("")
...
Markdown(metrics_md)
```
# Look and see if you want to keep this 

```{python}
#setting 5 folds to be similar to DNN
kf = KFold(n_splits=5, shuffle=True, random_state=42)
...
#plotting the losses across each fold
for i in range(5):
